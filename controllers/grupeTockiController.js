...
const getAll = (req, res) => {
  const sql = "SELECT * FROM grupeTocki WHERE deleted IS NOT TRUE;";
...
const update = async (req, res) => {
  ...
  const sql = newDbRow.uid
    ? "UPDATE grupeTocki SET naziv = ? WHERE uid = ?;"
    : "INSERT INTO grupeTocki(naziv) VALUES (?);";
...
const markDeleted = (req, res) => {
  const sql = "UPDATE grupeTocki SET deleted = TRUE WHERE uid = ?;";
