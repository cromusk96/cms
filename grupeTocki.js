let objects;
let filteredObjects;
//MOZDA skupiti ova tri file-a u jedan. (grupeTocki, podloga i osuncanost)
...
function deleteInDB(index) {
  httpDelete("/cms/api/v1/grupeTocki/" + filteredObjects[index].uid)
    .then(getData)
    .then(displayData);
}
...
function saveButton(index) {
  ...
  httpPost("/cms/api/v1/grupeTocki", object).then(getData).then(displayData);
